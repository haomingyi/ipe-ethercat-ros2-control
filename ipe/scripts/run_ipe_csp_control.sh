...
log_file="${log_dir}/ros2_csp_control_latest.log"
...
if pgrep -af 'single_joint_lab|ipe_three_mode_lab|ipe_joint_state_publisher|ipe_ros2_control_node' \
    ...
    echo "Stop it with quit or Ctrl+C before starting ROS 2 CSP control." >&2
...
mkdir -p "${log_dir}"
cd "${project_dir}"
stdbuf -oL -eL ros2 launch ethercat_master ipe_csp_control.launch.py \
    2>&1 | tee "${log_file}"
